...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    driveNameAndUrlModel * model123 = [[SqliteTools sharedSqliteTools] isExistAppWithName:model.conversation.conversationId];
    if(model123){
        model.title = model123.driverName;
        model.avatarURLPath = HTTP_URLIP(model123.driverHeadImg);
    }
    cell.model = model;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(conversationListViewController:didSelectConversationModel:)]) {
        ...
        [parameters setObject:model.conversation.conversationId forKey:@"hxDriverName"];
        [HttpRequest postWithURL:HTTP_URLIP(@"chat/getDriverNick") params:parameters andNeedHub:YES success:^(NSDictionary *responseObject) {
            
            driveNameAndUrlModel * model1 = [[driveNameAndUrlModel alloc]init];
            [model1 setValuesForKeysWithDictionary:responseObject[@"data"]];
            model1.driverHXID = model.conversation.conversationId;
            model.title = model1.driverName;
            model.avatarURLPath = HTTP_URLIP(model1.driverHeadImg);
            [GVUserDefaults standardUserDefaults].DriverHXAccount = model1.driverName;
            if(!IsStrEmpty(model1.driverHeadImg)){
            [GVUserDefaults standardUserDefaults].DriverHXHeadImg = model1.driverHeadImg;
            }else{
            [GVUserDefaults standardUserDefaults].DriverHXHeadImg = @"";
            model1.driverHeadImg = @"";
            }
    ...
    if (lastMessage) {
        ...
        switch (messageBody.type) {
            case EMMessageBodyTypeImage:{
                latestMessageTitle = @"[图片]";//NSEaseLocalizedString(@"message.image1", @"[image]");
            } break;
            ...
            case EMMessageBodyTypeVoice:{
                latestMessageTitle = @"[语音]";//NSEaseLocalizedString(@"message.voice1", @"[voice]");
            } break;
            case EMMessageBodyTypeLocation: {
                latestMessageTitle = @"[位置]";//NSEaseLocalizedString(@"message.location1", @"[location]");
            } break;
            case EMMessageBodyTypeVideo: {
                latestMessageTitle = @"[视频]";//NSEaseLocalizedString(@"message.video1", @"[video]");
            } break;
