...
+(NSAttributedString*)createStrLabelTextWith:(NSString*)textStr and:(NSString*)fontStr andFont:(UIFont*)font with:(NSString*)colorStr and:(UIColor*)color{

    NSMutableAttributedString *str = [[NSMutableAttributedString alloc]initWithString:textStr];
    if(colorStr){
    NSRange range = [textStr rangeOfString:colorStr];
    //设置单位长度内的内容显示成色
    [str addAttribute:NSForegroundColorAttributeName value:color range:range];
    }
    if(fontStr){
    NSRange range1 = [textStr rangeOfString:fontStr];
...
+(UIBarButtonItem*)createRightNavItem:(NSString*)string andFont:(UIFont*)font andSelecter:(SEL)sel andTarget:(id)target{
    CGSize size = [self countTxtWith:string andWithFont:font with:SCREEN_WIDTH];
    UIButton *releaseButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    releaseButton.frame = CGRectMake(0, 0, size.width+4, 30);
    [releaseButton setTitle:string forState:normal];
    ...
    [releaseButton addTarget:target action:sel forControlEvents:UIControlEventTouchUpInside];
