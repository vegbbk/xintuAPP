...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self = [super initWithFrame:frame]){
        ...
        Namelabel.font = FontSize(fontSizeLJJ);
        [self addSubview:Namelabel];
        
        textField = [[UITextField alloc]init];
        textField.font = FontSize(fontSizeLJJ);
...
- (void)layoutSubviews{
    ...
    imageView.image = [UIImage imageNamed:_headImageName];
     CGSize titleSize = [_nameLabelStr boundingRectWithSize:CGSizeMake(MAXFLOAT, 30) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:FontSize(fontSizeLJJ)} context:nil].size;
    if(_headImageName){
