...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int j = 0; j < 5; j ++)
    {
        ...
        imageView.frame = CGRectMake(j*imageW+2*j, 2, imageW, imageW);
...
-(void)tapGR:(UITapGestureRecognizer *)tapGR{
    ...
    int X = (int) point.x/(imageW+2);
    
    self.starForegroundView.frame = CGRectMake(0, 0, (X+1)*imageW+(X+1)*2, imageW+4);
    [self addSubview:self.starForegroundView];
    if(X>4){
        X=4;
    }
    [self.delegate selectNumber:X and:self.tag];
