...
@interface calendarSelectView ()<UIPickerViewDataSource,UIPickerViewDelegate>
{
...
-(void)initData{
    ...
    self.model.moth = [NSString stringWithFormat:@"%.2d",[array[1] intValue]];
    self.model.day = array[2];
    
    _yearArray = [NSMutableArray array];
    NSString *yearSystem = array[0];
    int yearCount = [yearSystem intValue];
    for (int i = yearCount; i>2015; i--) {
...
-(void)initView{
    ...
    UIButton * cancleBtn = [[UIButton alloc] initWithFrame:CGRectMake(20, 10, 60, 30)];
    [cancleBtn setTitle:@"取消" forState:0];
    [cancleBtn setTitleColor:RGB170 forState:0];
    cancleBtn.layer.cornerRadius = 4;
    cancleBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    cancleBtn.layer.masksToBounds = YES;
    cancleBtn.layer.borderColor = RGB170.CGColor;
    cancleBtn.layer.borderWidth = 1;
    [cancleBtn addTarget:self action:@selector(cancelClick) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:cancleBtn];
    
    UIButton * sureBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width -80, 10, 60, 30)];
    ...
    sureBtn.backgroundColor = MAINThemeColor;
    ...
    int year = [yearRow intValue]-2016;
...
//    selectDay = [NSString stringWithFormat:@"%d",day];
    ...
    UIView * selectViewBac = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width-50, 30)];
...
-(void)cancelClick{
    [self.delegate FreeCancelViewData];
...
-(void)sureBtnClick{
    [self removeFromSuperview];
    [self.delegate FreeDataSelectClick:selectYear with:selectMonth];
}
...
- (CGFloat) pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component{
    if (component==0) {
        return  (self.bounds.size.width-50)/2;
    } else{
        return  (self.bounds.size.width-50)/2;
    }
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    UILabel *text = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, (self.bounds.size.width-50)/2, 30)];
//    if (component == 2) {
